...
import { Badge, Btn, Card, Notice, Page, Stat, cx } from "../../ui/Primitives";
...
export default function ParentAttendance() {
  ...
  return (
    <Page kicker="الحضور والغياب" title="الحضور والغياب" desc="سجلّ الحضور اليومي لابنك، وتصلك رسالة فورية عند تسجيل أي غياب." icon={CalendarCheck}>
      ...
      {absent.length > 0 && (
        <Notice tone="warn" title={`${absent.length} أيام غياب في الفترة`} action={<Btn size="sm" variant="ghost" icon={Send} onClick={() => { dispatch({ type: "newThread", from: user.id, to: "adm-school", subject: `تبرير غياب ${child.name}`, text: `أرجو اعتماد عذر غياب ${child.name} في التواريخ: ${absent.join("، ")}.` }); toast("أُرسل طلب تبرير الغياب إلى الإدارة"); }}>أرسل تبرير الغياب</Btn>}>
